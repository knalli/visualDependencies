...
public class PopupMousePlugin extends AbstractPopupGraphMousePlugin implements MouseListener {
	...
	protected TriggerViewData data;
	...
	protected void handlePopup(final MouseEvent e) {
		final VisualizationViewer<DatabaseTrigger, Relation> vv = (VisualizationViewer<DatabaseTrigger, Relation>) e
				.getSource();
		final Point2D p = e.getPoint();// vv.getRenderContext().getBasicTransformer().inverseViewTransform(e.getPoint());

		final GraphElementAccessor<DatabaseTrigger, Relation> pickSupport = vv.getPickSupport();
		if (pickSupport != null) {

			if (enableVertexPopupMenu) {
				final DatabaseTrigger vertex = pickSupport.getVertex(vv.getGraphLayout(), p.getX(), p.getY());
				if (vertex != null) {
